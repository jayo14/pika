...
const db = new better_sqlite3_1.default('./welcomebot.db');
...
const client = new discord_js_1.Client({
    intents: [
        ...
        discord_js_1.GatewayIntentBits.GuildMembers, // For guild member events (like guildMemberAdd)
        discord_js_1.GatewayIntentBits.GuildMessageReactions, // For reaction tracking
        discord_js_1.GatewayIntentBits.DirectMessages, // For DM reactions
        discord_js_1.GatewayIntentBits.GuildMessageTyping, // For typing indicators (engagement)
        discord_js_1.GatewayIntentBits.GuildInvites, // For invite tracking
        discord_js_1.GatewayIntentBits.GuildScheduledEvents, // For scheduled events
    ],
    partials: [discord_js_1.Partials.User, discord_js_1.Partials.GuildMember, discord_js_1.Partials.Channel, discord_js_1.Partials.Message, discord_js_1.Partials.Reaction, discord_js_1.Partials.GuildScheduledInvite], // Helps with uncached members/channels/messages/reactions/events/invites
});
...
function hasUserOptedOutOfDMs(userId) {
...
function setUserDMOptOut(userId, optOut) {
    db.prepare(`
...
function setWelcomeSettings(guildId, backgroundImage, enabled) {
    db.prepare(`
...
function getRoleWelcomeSettings(guildId, roleId) {
...
function setRoleWelcomeSettings(guildId, roleId, message, channelId, enabled) {
    db.prepare(`
...
function getWelcomeComponents(guildId) {
...
function addWelcomeComponent(guildId, componentType, label, value, description, emoji, style, placeholder, minValues, maxValues, disabled, position) {
    db.prepare(`
...
function getAbTestVariants(testId) {
    return db.prepare(`
...
function logAbTestExposure(testId, variantId, userId, guildId) {
    db.prepare(`
...
function recordAbTestEngagement(exposureId, engagementType) {
...
function getAbTestStats(testId) {
    return db.prepare(`
    SELECT
      v.id as variant_id,
...
async function initializeInviteTracking(guildId) {
    try {
        ...
        invites.forEach(invite => {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(guildId, invite.code, invite.inviter ? invite.inviter.id : null, invite.uses, invite.maxUses || 0, invite.temporary ? 1 : 0, isVanity ? 1 : 0, invite.createdAt?.toISOString() || null, invite.expiresAt?.toISOString() || null);
            }
            else {
...
function isLikelyVanityUrl(code) {
...
async function handleInviteUsage(member) {
    try {
        // Get current invites
...
async function findUsedInvite(guildId, newInvites) {
    try {
...
function updateInviteCache(newInvites) {
...
async function checkForSuspiciousInvite(guildId, inviteCode, member) {
    try {
...
async function createWelcomeCard(member) {
    const width = 1024;
    const height = 500;
    const canvas = canvas_1.Canvas.createCanvas(width, height);
    ...
    try {
        ...
        const response = await (0, undici_1.request)(avatarURL);
        const arrayBuffer = await new Response(response.body).arrayBuffer();
        avatar = await (0, canvas_1.loadImage)(Buffer.from(arrayBuffer));
    }
    catch (error) {
        console.error('Failed to load avatar:', error);
        // Use default avatar if fetch fails
        avatar = await (0, canvas_1.loadImage)(member.user.defaultAvatarURL);
    }
    // Draw avatar circle
    ...
    try {
        ...
        if (useWelcomeCard) {
            try {
                ...
                const attachment = new discord_js_1.AttachmentBuilder(welcomeCard, { name: 'welcome-card.png' });
...
function buildActionRows(components) {
    ...
    for (const component of components) {
        ...
        if (componentBuilder) {
            ...
            if (currentRow.length >= 5) {
                actionRows.push(new discord_js_1.ActionRowBuilder().addComponents(currentRow));
                currentRow = [];
            }
...
async function sendRegularWelcome(member, welcomeSettings, roleSpecificSetting, defaultChannel, welcomeComponents, exposureId = null) {
    ...
    if (interaction.commandName === 'welcomesetup') {
        ...
        `).run(gid, name, description ?? null, isActive);
                ...
                if (variantSubcommand === 'add') {
                    ...
                    const result = db.prepare(sql).run(test.id, variantName, message ?? null, weight, isControl ? 1 : 0);
                    await interaction.reply({
        ...
        `).all(gid);
                if (vanityInvites.length > 0) {
                    response += `\nTop Vanity URLs:\n`;
                    vanityInvites.forEach((invite, index) => {
        ...
        `).all(gid);
                if (recentSuspicious.length > 0) {
                    ...
                    recentSuspicious.forEach((suspect, index) => {
